...
@router.get("/employee/by-email/{email}", response_model=EmployeeOut)
...
@router.get("/employee/{manager_id}/reports", response_model=DirectReports)
def get_employee_direct_reports(manager_id: int):
    result = services.get_direct_reports(manager_id)
    if result is None:
        raise HTTPException(status_code=404, detail=f"Employee with id {manager_id} not found")
    return result