...
def run_tests():
    ...
    all_emps = client.get("/employees/search?limit=100", headers=admin_headers).json()
    emp5_public_id = None
    emp6_public_id = None
    for emp in all_emps["items"]:
        if emp["employee_code"] in ("EMP-0004", "EMP-1005"):
            emp5_public_id = emp["public_id"]
        if emp["employee_code"] in ("EMP-0006", "EMP-1006"):
            emp6_public_id = emp["public_id"]
    if not emp5_public_id and len(all_emps["items"]) > 1:
        emp5_public_id = all_emps["items"][1]["public_id"]
    if not emp6_public_id and len(all_emps["items"]) > 2:
        emp6_public_id = all_emps["items"][2]["public_id"]
    assert emp5_public_id is not None, "Could not find employee for test"
    assert emp6_public_id is not None, "Could not find second employee for test"
    print(f"[INFO] Using emp5 UUID: {emp5_public_id}, emp6 UUID: {emp6_public_id}")

    # Clean up any residual check-in for emp5 today
    today_str = datetime.date.today().isoformat()
    for r in att_repo.get_all():
        if r.get("employee_public_id") == emp5_public_id and r.get("date") == today_str:
    ...
    res = client.post("/attendance/check-in", json=checkin_payload, headers=admin_headers)
    ...
    dup_res = client.post("/attendance/check-in", json=checkin_payload, headers=admin_headers)
    ...
    out_res = client.post("/attendance/check-out", json=checkout_payload, headers=admin_headers)
