...
def get_all_designations(skip: int = 0, limit: int | None = None, db: Session = None) -> dict:  # type: ignore
    def _fetch():
        ...
        formatted = [d.to_dict() for d in items]
        return {"total": total, "skip": skip, "limit": limit, "items": formatted}

    cache_key = f"list_{skip}_{limit}"
    return get_cached_or_compute("designations", cache_key, _fetch)
...
def get_designation_by_public_id(public_id: str, db: Session) -> dict | None:
    ...
    return get_cached_or_compute("designations", public_id, _fetch)
