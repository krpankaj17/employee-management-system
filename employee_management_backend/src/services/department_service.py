...
def _resolve_head_employee_uuid(public_id: str | None, db: Session) -> tuple[int | None, str | None]:
    ...
    if not public_id or str(public_id).strip() in ("", "none", "null"):
        return None, None
...
def _format_department(dept, db: Session) -> dict:
    """Formats Department ORM object into dictionary with head_employee_public_id UUID, head name, and employee count."""
    head_public_id = None
    head_name = None
    if dept.head_employee_id:
        ...
        if head_emp and hasattr(head_emp, "public_id"):
            ...
            head_name = f"{head_emp.first_name} {head_emp.last_name}".strip()

    emp_count = 0
    if hasattr(dept, "dept_id") and dept.dept_id is not None:
        from sqlalchemy import select, func
        from models.employee import Employee
        count_val = db.scalar(
            select(func.count(Employee.emp_id)).where(Employee.dept_id == dept.dept_id)
        )
        emp_count = count_val or 0
...
def get_all_departments(skip: int = 0, limit: int | None = None, db: Session = None) -> dict:  # type: ignore
    ...
    return get_cached_or_compute("departments", cache_key, _fetch)
...
def get_department_by_public_id(public_id: str, db: Session) -> dict | None:
    ...
    return get_cached_or_compute("departments", public_id, _fetch)
