...
def create_project(payload: ProjectCreateIn, db: Session) -> dict[str, Any]:
    ...
    head_emp_id = None
    target_head_pid = payload.project_head_public_id or payload.head_employee_public_id
    if target_head_pid:
        head = emp_repo.get_by_public_id(target_head_pid, db=db)
        if not head:
            return {"ok": False, "error": "not_found", "message": f"Project head employee '{target_head_pid}' not found"}
...
def update_project(public_id: str, payload: ProjectUpdateIn, db: Session) -> dict[str, Any]:
    ...
    if "project_head_public_id" in fields_set or "head_employee_public_id" in fields_set:
        target_head_pid = (
            payload.project_head_public_id
            if "project_head_public_id" in fields_set
            else payload.head_employee_public_id
        )
        if target_head_pid in (None, "", "none", "null"):
            p.project_head_id = None
            p.project_head = None
        else:
            head = emp_repo.get_by_public_id(target_head_pid, db=db)
            if not head:
                return {"ok": False, "error": "not_found", "message": f"Project head '{target_head_pid}' not found"}
            p.project_head_id = cast(int, head.emp_id)
            p.project_head = head
    elif payload.project_head_public_id is not None or payload.head_employee_public_id is not None:
        target_head_pid = (
            payload.project_head_public_id
            if payload.project_head_public_id is not None
            else payload.head_employee_public_id
        )
        if target_head_pid in ("", "none", "null"):
            p.project_head_id = None
            p.project_head = None
        else:
            head = emp_repo.get_by_public_id(target_head_pid, db=db)
    ...
    res_dict = updated.to_dict()
    res_dict["members"] = [m.to_dict() for m in updated.members] if updated.members else []
    return {"ok": True, "project": res_dict}
