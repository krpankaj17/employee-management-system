...
def get_all_active_salaries(db: Session) -> list[dict[str, Any]]:
    """Retrieves all active salary structures across all employees."""
    salaries = repo.list_all_active_salaries(on_date=datetime.date.today(), db=db)
    return [s.to_dict() for s in salaries]


def update_salary_structure(
    public_id: str, payload: SalaryCreateIn, db: Session
) -> dict[str, Any]:
    """Updates an existing salary structure revision."""
    salary = repo.get_salary_by_public_id(public_id, db=db)
    if not salary:
        return {"ok": False, "error": "not_found", "message": f"Salary revision '{public_id}' not found"}

    try:
        salary.basic_salary = payload.basic_salary
        salary.currency = payload.currency
        salary.effective_from = payload.effective_from

        # Clear existing components
        for c in list(salary.components):
            db.delete(c)
        db.flush()
        ...
        salary.net_salary = payload.basic_salary + total_earnings - total_deductions
        db.commit()
        db.refresh(salary)
        log_action("SALARY_UPDATED", f"Updated salary structure '{public_id}'")
        return {"ok": True, "salary": salary.to_dict()}
    except Exception as e:
        db.rollback()
        return {"ok": False, "error": "db_error", "message": str(e)}


def delete_salary_structure(public_id: str, db: Session) -> dict[str, Any]:
    """Deletes an erroneous salary structure revision."""
    salary = repo.get_salary_by_public_id(public_id, db=db)
    if not salary:
        return {"ok": False, "error": "not_found", "message": f"Salary revision '{public_id}' not found"}

    try:
        repo.delete_salary(salary, db=db)
        log_action("SALARY_DELETED", f"Deleted salary structure '{public_id}'")
        return {"ok": True, "message": "Salary structure revision successfully deleted"}
    except Exception as e:
        db.rollback()
        return {"ok": False, "error": "db_error", "message": str(e)}
...
def update_bank_detail(public_id: str, payload: BankDetailUpdateIn, db: Session) -> dict[str, Any]:
    """Updates an existing bank account record."""
    bank = repo.get_bank_detail_by_public_id(public_id, db=db)
    if not bank:
        return {"ok": False, "error": "not_found", "message": f"Bank detail with public_id '{public_id}' not found"}

    try:
        if payload.is_primary and not bank.is_primary:
            repo.clear_primary_bank_details(cast(int, bank.emp_id), db=db)

        if payload.bank_name is not None:
            bank.bank_name = payload.bank_name.strip()
        if payload.branch_name is not None:
            bank.branch_name = payload.branch_name.strip() if payload.branch_name else None
        if payload.account_number is not None:
            bank.account_number = payload.account_number.strip()
        if payload.routing_code is not None:
            bank.routing_code = payload.routing_code.strip().upper()
        ...
        db.commit()
        db.refresh(bank)
        log_action("BANK_DETAIL_UPDATED", f"Updated bank account '{public_id}'")
        return {"ok": True, "bank_detail": bank.to_dict()}
    except Exception as e:
        db.rollback()
        return {"ok": False, "error": "db_error", "message": str(e)}


def delete_bank_detail(public_id: str, db: Session) -> dict[str, Any]:
    """Deletes a bank detail record."""
    bank = repo.get_bank_detail_by_public_id(public_id, db=db)
    if not bank:
        return {"ok": False, "error": "not_found", "message": f"Bank detail with public_id '{public_id}' not found"}

    try:
        repo.delete_bank_detail(bank, db=db)
        log_action("BANK_DETAIL_DELETED", f"Deleted bank account '{public_id}'")
        return {"ok": True, "message": "Bank detail successfully deleted"}
    except Exception as e:
        db.rollback()
        return {"ok": False, "error": "db_error", "message": str(e)}
...
def get_payroll_runs_paginated(
    ...
    if employee_public_id and str(employee_public_id).strip().lower() in ("undefined", "null", "none", ""):
...
def delete_payroll_run(payroll_public_id: str, db: Session) -> dict[str, Any]:
    """Cancels / deletes an un-disbursed payroll run."""
    run = repo.get_payroll_run_by_public_id(payroll_public_id, db=db)
    if not run:
        return {
            "ok": False,
    ...
    if run.payment_status == "paid":
        return {
            "ok": False,
            "error": "conflict",
            "message": "Cannot delete a disbursed/paid payroll run",
        }

    try:
        repo.delete_payroll_run(run, db=db)
        log_action("PAYROLL_RUN_DELETED", f"Deleted payroll run '{payroll_public_id}'")
        return {"ok": True, "message": "Payroll run successfully deleted"}
    except Exception as e:
        db.rollback()
        return {"ok": False, "error": "db_error", "message": str(e)}
