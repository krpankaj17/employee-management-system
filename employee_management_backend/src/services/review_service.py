...
def delete_performance_review(public_id: str, db: Session) -> dict[str, Any]:
    """Deletes a performance review."""
    r = repo.get_review_by_public_id(public_id, db=db)
    if not r:
        return {"ok": False, "error": "not_found", "message": f"Review '{public_id}' not found"}

    repo.delete_review(r, db=db)
    log_action("REVIEW_DELETED", f"Performance review '{public_id}' deleted")
    return {"ok": True, "message": "Performance review successfully deleted"}
