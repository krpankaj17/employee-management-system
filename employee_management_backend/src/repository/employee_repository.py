...
def search(
    ...
    if public_id and public_id.strip():
        try:
            uuid_val = uuid.UUID(public_id.strip())
            conditions.append(Employee.public_id == uuid_val)
        except (ValueError, AttributeError):
            return 0, []
    if email and email.strip():
        conditions.append(func.lower(Employee.email) == email.strip().lower())
    if employee_code and employee_code.strip():
        conditions.append(func.lower(Employee.employee_code) == employee_code.strip().lower())
    if reporting_manager_id is not None:
        conditions.append(Employee.reporting_manager_id == reporting_manager_id)
...
def get_by_public_id(public_id: str, db: Session | None = None) -> Employee | None:
    ...
    if not public_id:
        return None
    try:
        uuid_val = uuid.UUID(str(public_id).strip())
    except (ValueError, AttributeError):
        return None
    ...
    if db is not None:
        stmt = (
            select(Employee)
            .options(*_eager_options())
            .where(Employee.public_id == uuid_val)
        )
        return db.scalar(stmt)
...
def get_by_phone(phone: str, db: Session | None = None) -> Employee | None:
    """Returns the Employee ORM object matching phone number."""
    if not phone:
        return None
    ...
    if db is not None:
        stmt = (
            select(Employee)
            .options(*_eager_options())
            .where(Employee.phone == clean_phone)
        )
        return db.scalar(stmt)

    with SessionLocal() as session:
        return get_by_phone(phone, db=session)
