...
def list_all_active_salaries(on_date: date | None, db: Session) -> list[Salary]:
    """Fetches all currently active salaries across employees."""
...
def delete_payroll_run(run: PayrollRun, db: Session) -> None:
    """Deletes a payroll run."""
    db.delete(run)
    db.commit()


def delete_salary(salary: Salary, db: Session) -> None:
    """Deletes a salary revision and cascade-deletes its components."""
    db.delete(salary)
    db.commit()


def delete_bank_detail(bank_detail: BankDetail, db: Session) -> None:
