...
def list_reviews(
    ...
    review_status: str | None = Query(None, alias="status", description="draft | submitted | acknowledged | finalized"),
    skip: int = Query(0, ge=0),
    limit: int | None = Query(None, gt=0),
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db),
):
    """Lists performance reviews with filtering.
    Employees can only see reviews where they are the reviewee or reviewer.
    HR/Admin with 'review:read' can list all reviews."""
    user_roles = [r.role_name for r in current_user.roles]
    has_perm = (
        current_user.has_exact_permission("review:read")
    ...
    if not has_perm:
        ...
        if employee_public_id and employee_public_id != user_emp_public_id:
            if reviewer_public_id != user_emp_public_id:
                raise HTTPException(
                    status_code=status.HTTP_403_FORBIDDEN,
                    detail="Access not granted: You do not have permission to view performance reviews for other employees.",
                )
        # If filtering by specific reviewer that is not self, check if caller is employee
        if reviewer_public_id and reviewer_public_id != user_emp_public_id:
            if employee_public_id != user_emp_public_id:
                raise HTTPException(
                    status_code=status.HTTP_403_FORBIDDEN,
                    detail="Access not granted: You do not have permission to view performance reviews conducted by other reviewers.",
                )
        # If no specific filter provided, default to reviews where caller is the reviewee
        if not employee_public_id and not reviewer_public_id:
...
def get_review(
    ...
    if (
        ...
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Access not granted: You do not have permission to view this review.",
        )
...
def update_review(
    ...
    """Updates review ratings/feedback. Allowed for reviewer, employee (for acknowledgement), or HR with 'review:create'."""
    existing = review_service.get_performance_review(public_id, db=db)
    if not existing:
        raise HTTPException(status_code=404, detail=f"Review '{public_id}' not found")

    has_perm = current_user.has_permission("review:create")
    user_emp = current_user.employee
    user_emp_public_id = str(user_emp.public_id) if user_emp else None

    if (
        not has_perm
        and existing.get("employee_public_id") != user_emp_public_id
        and existing.get("reviewer_public_id") != user_emp_public_id
    ):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
...
def delete_review(
    ...
    """Deletes a performance review. Allowed for Reviewer or users with 'review:delete' or 'review:create'."""
    existing = review_service.get_performance_review(public_id, db=db)
    if not existing:
        raise HTTPException(status_code=404, detail=f"Review '{public_id}' not found")

    has_perm = current_user.has_permission("review:delete") or current_user.has_permission("review:create")
    user_emp = current_user.employee
    user_emp_public_id = str(user_emp.public_id) if user_emp else None

    if not has_perm and existing.get("reviewer_public_id") != user_emp_public_id:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="You do not have permission to delete this review",
        )

    res = review_service.delete_performance_review(public_id, db=db)
    if not res["ok"]:
        code = 404 if res["error"] == "not_found" else 400
        raise HTTPException(status_code=code, detail=res["message"])
    return {"details": res["message"]}
