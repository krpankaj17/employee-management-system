...
def update_salary_revision(
    public_id: str,
    payload: SalaryCreateIn,
    db: Session = Depends(get_db),
):
    """Updates an existing salary structure revision (Admin/HR only)."""
    result = payroll_service.update_salary_structure(public_id, payload, db=db)
    if not result["ok"]:
...
def delete_salary_revision(
    ...
    """Deletes an erroneous salary structure revision (Admin/HR only)."""
    result = payroll_service.delete_salary_structure(public_id, db=db)
    if not result["ok"]:
        code = 404 if result.get("error") == "not_found" else 400
        raise HTTPException(status_code=code, detail=result["message"])
...
def get_all_salaries(
    ...
    """Retrieves all active salary structures across all employees (Admin/HR only)."""
    return payroll_service.get_all_active_salaries(db=db)


@salary_router.get(
    "/me",
    response_model=SalaryHistoryOut,
    summary="Get current employee's salary details and history",
)
def get_my_salaries(
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db),
):
    """Retrieves active salary structure and complete history for the currently authenticated employee."""
    ...
    result = payroll_service.get_employee_salary_history(user_emp_public_id, db=db)
    if not result["ok"]:
        code = 404 if result.get("error") == "not_found" else 400
        raise HTTPException(status_code=code, detail=result["message"])
    return result["data"]
...
def get_employee_salaries(
    ...
    if not has_perm and user_emp_public_id != employee_public_id:
        raise HTTPException(
            ...
            detail="Access not granted: You do not have permission to view salary details for other employees.",
        )
...
@bank_router.post(
    ...
    status_code=status.HTTP_201_CREATED,
    dependencies=[Depends(require_permission("payroll:update"))],
)
def add_employee_bank_account(
    payload: BankDetailIn,
    db: Session = Depends(get_db),
):
    """Adds a bank account for salary disbursement. Requires 'payroll:update' permission (Admin/HR only)."""
...
def update_employee_bank_account(
    public_id: str,
    payload: BankDetailUpdateIn,
    db: Session = Depends(get_db),
):
    """Updates an existing bank account record. Requires 'payroll:update' permission (Admin/HR only)."""
    result = payroll_service.update_bank_detail(public_id, payload, db=db)
    if not result["ok"]:
        code = 404 if result.get("error") == "not_found" else 400
        raise HTTPException(status_code=code, detail=result["message"])
    return result["bank_detail"]
...
def delete_employee_bank_account(
    public_id: str,
    db: Session = Depends(get_db),
):
    """Deletes a bank account record. Requires 'payroll:update' permission (Admin/HR only)."""
    result = payroll_service.delete_bank_detail(public_id, db=db)
    if not result["ok"]:
        code = 404 if result.get("error") == "not_found" else 400
        raise HTTPException(status_code=code, detail=result["message"])
    return {"details": result["message"]}
...
@bank_router.get(
    ...
    summary="Get current employee's bank details",
)
def get_my_bank_accounts(
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db),
):
    """Lists registered bank accounts for the currently authenticated employee."""
    if not current_user.employee:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="No employee profile found for the current user",
    ...
    user_emp_public_id = str(current_user.employee.public_id)
    result = payroll_service.get_employee_bank_details(user_emp_public_id, db=db)
    if not result["ok"]:
        code = 404 if result.get("error") == "not_found" else 400
        raise HTTPException(status_code=code, detail=result["message"])
    return result["items"]
...
def list_employee_bank_accounts(
    ...
    has_perm = (
        current_user.has_permission("payroll:read")
    ...
    if not has_perm and user_emp_public_id != employee_public_id:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Access not granted: You do not have permission to view bank accounts for other employees.",
        )
...
def get_payslip(
    ...
    if not has_perm and user_emp_public_id != payslip.get("employee_public_id"):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Access not granted: You do not have permission to access payslips for other employees.",
        )
...
def delete_payroll_run(
    ...
    """Cancels / deletes an un-disbursed payroll run (Admin/HR only)."""
    result = payroll_service.delete_payroll_run(public_id, db=db)
    if not result["ok"]:
        code = 404 if result.get("error") == "not_found" else 400
        raise HTTPException(status_code=code, detail=result["message"])
    return {"details": result["message"]}
