...
class BankDetailUpdateIn(BaseModel):
    bank_name: str | None = Field(default=None, min_length=2, max_length=150, description="Full bank institution name")
    branch_name: str | None = Field(default=None, max_length=150, description="Branch location/name")
    account_number: str | None = Field(default=None, min_length=4, max_length=34, description="Bank account number")
    routing_code: str | None = Field(default=None, min_length=4, max_length=20, description="IFSC / SWIFT / Sort / Routing code")
    account_type: Literal["savings", "current"] | None = Field(default=None, description="'savings' or 'current'")
    is_primary: bool | None = Field(default=None, description="Whether this is the employee's primary salary disbursement account")
...
class PayrollRunOut(BaseModel):
    ...
    @field_validator("pay_period_start", "pay_period_end", "payment_date", mode="before")
    @classmethod
    def format_dates(cls, value: Any) -> str | None:
        if value is None:
            return None
