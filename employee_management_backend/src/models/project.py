...
class Project(Base):
    ...
    status: Mapped[str] = mapped_column(String(20), default="planning", index=True)  # planning, active, on_hold, completed, cancelled
    ...
    def to_dict(self) -> dict:
        head_name = (
            f"{self.project_head.first_name} {self.project_head.last_name}".strip()
            if self.project_head
            else None
        )
        head_pid = str(self.project_head.public_id) if self.project_head else None
        m_count = len(cast(list, self.members)) if self.members else 0

        return {
            "public_id": str(self.public_id),
            "project_name": self.project_name,
            "description": self.description,
            "project_head_public_id": head_pid,
            "project_head_name": head_name,
            "head_employee_public_id": head_pid,
...
class ProjectMember(Base):
    ...
    project_id: Mapped[int] = mapped_column(
        BigInteger, ForeignKey("projects.project_id", ondelete="CASCADE"), primary_key=True, index=True
    )
    employee_id: Mapped[int] = mapped_column(
        BigInteger, ForeignKey("employees.emp_id", ondelete="CASCADE"), primary_key=True, index=True
    )
