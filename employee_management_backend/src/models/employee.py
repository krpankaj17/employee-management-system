...
class Employee(Base):
    ...
    public_id: Mapped[str] = mapped_column(
        PG_UUID(as_uuid=True), server_default=text("gen_random_uuid()"), index=True
    )
    ...
    employee_status: Mapped[str] = mapped_column(String(20), default="active", index=True)
    employment_type: Mapped[str] = mapped_column(String(20), default="full_time")
    dept_id: Mapped[int | None] = mapped_column(
        BigInteger, ForeignKey("departments.dept_id", ondelete="RESTRICT"), nullable=True, index=True
    )
    designation_id: Mapped[int | None] = mapped_column(
        BigInteger, ForeignKey("designations.designation_id", ondelete="SET NULL"), nullable=True, index=True
    )
    ...
    timezone: Mapped[str | None] = mapped_column(String(50), default="UTC", nullable=True)
    ...
    user_id: Mapped[int] = mapped_column(
        BigInteger, ForeignKey("users.user_id", ondelete="RESTRICT"), unique=True, index=True
    )
    ...
    __table_args__ = (
        Index("idx_employees_email_lower", func.lower(email)),
    )
    ...
    def department_name(self) -> str | None:
        """Computed property: resolves internal dept_id → department.dept_name."""
        if self.department and hasattr(self.department, "dept_name"):
            return str(self.department.dept_name)
        return None

    @property
    def designation_name(self) -> str | None:
        """Computed property: resolves internal designation_id → designation.title."""
        if self.designation and hasattr(self.designation, "title"):
            return str(self.designation.title)
        return None
