...
class Attendance(Base):
    ...
    __table_args__ = (
        Index("idx_attendance_emp_date", "emp_id", "date"),
    )
    ...
    @property
    def check_in_time(self) -> str | None:
        if self.check_in and hasattr(self.check_in, "isoformat"):
            return self.check_in.isoformat()
        return str(self.check_in) if self.check_in else None

    @property
    def check_out_time(self) -> str | None:
        if self.check_out and hasattr(self.check_out, "isoformat"):
            return self.check_out.isoformat()
        return str(self.check_out) if self.check_out else None

    def to_dict(self) -> dict:
        cin_iso = (
            self.check_in.isoformat()
            if self.check_in and hasattr(self.check_in, "isoformat")
            else (str(self.check_in) if self.check_in else None)
        )
        cout_iso = (
            self.check_out.isoformat()
            if self.check_out and hasattr(self.check_out, "isoformat")
            else (str(self.check_out) if self.check_out else None)
        )
        cin = (
            self.check_in.strftime("%H:%M:%S")
            if self.check_in and getattr(self.check_in, "microsecond", 1) == 0
            else cin_iso
        )
        cout = (
            self.check_out.strftime("%H:%M:%S")
            if self.check_out and getattr(self.check_out, "microsecond", 1) == 0
            else cout_iso
        )
        return {
            ...
            "check_in": cin,
            "check_in_time": cin_iso,
            "check_out": cout,
            "check_out_time": cout_iso,
            "timezone": self.timezone or "UTC",
            "work_mode": self.work_mode,
            "status": self.status,
            "total_hours": float(cast(Decimal, self.total_hours)) if self.total_hours is not None else 0.0,
            "notes": self.notes,
            "is_late": bool(self.notes and "[Late Arrival" in self.notes),
        }
