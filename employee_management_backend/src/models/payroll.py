...
class Salary(Base):
    ...
    __table_args__ = (
        Index("idx_salaries_effective", "effective_from", "effective_to"),
    )
    ...
    def employee_name(self) -> str | None:
        if self.employee:
            ...
            return full or getattr(self.employee, "display_name", None)
        return None

    @property
    def employee_code(self) -> str | None:
        if self.employee:
            return getattr(self.employee, "employee_code", None)
        return None

    @property
    def department_name(self) -> str | None:
        if self.employee and getattr(self.employee, "department", None):
            return getattr(self.employee.department, "dept_name", None)
        return None
...
class SalaryComponent(Base):
    ...
    salary_id: Mapped[int] = mapped_column(
        BigInteger, ForeignKey("salaries.salary_id", ondelete="CASCADE"), index=True
    )
...
class PayrollRun(Base):
    ...
    salary_id: Mapped[int | None] = mapped_column(
        BigInteger, ForeignKey("salaries.salary_id", ondelete="SET NULL"), nullable=True, index=True
    )
    ...
    payment_status: Mapped[str] = mapped_column(String(20), default="pending", index=True)  # 'pending', 'processed', 'paid', 'failed'
