...
def test_payroll_and_salary_flow(client: TestClient, admin_headers, employee_auth, employee_headers):
    ...
    # 2b. List all active salaries across company (Admin only)
    all_sal_res = client.get("/salaries", headers=admin_headers)
    assert all_sal_res.status_code == 200
    all_sal_list = all_sal_res.json()
    assert isinstance(all_sal_list, list)
    assert any(s["employee_public_id"] == emp_pid for s in all_sal_list)
    matching = next(s for s in all_sal_list if s["employee_public_id"] == emp_pid)
    assert matching["employee_name"] is not None
    assert len(matching["components"]) == 2

    # 3. Get Employee Salary History (as Admin and self-service /me)
    ...
    # Self-service salary history
    me_sal_res = client.get("/salaries/me", headers=employee_headers)
    assert me_sal_res.status_code == 200
    assert "history" in me_sal_res.json()

    # 4. Add Bank Details (Employee blocked, Admin allowed)
    bank_payload = {
        "employee_public_id": emp_pid,
        "bank_name": "HDFC Bank",
    ...
    # Employee blocked from adding bank details (HR/Admin only)
    emp_bank_res = client.post("/bank-details", json=bank_payload, headers=employee_headers)
    assert emp_bank_res.status_code == 403

    # Admin successfully adds bank details
    ...
    # 5. List Bank Details (by UUID and self-service /me)
    ...
    me_banks = client.get("/bank-details/me", headers=employee_headers)
    assert me_banks.status_code == 200
    assert len(me_banks.json()) >= 1
