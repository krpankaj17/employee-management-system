...
def test_project_lifecycle_flow(client: TestClient, admin_headers, employee_auth):
    ...
    # 3a. Remove Project Head via empty string
    remove_head_res = client.put(
        f"/projects/{proj_pid}",
        json={"project_head_public_id": ""},
        headers=admin_headers,
    )
    assert remove_head_res.status_code == 200
    assert remove_head_res.json()["project_head_public_id"] is None
    assert remove_head_res.json()["head_employee_name"] is None

    # 3b. Re-assign Project Head to employee
    reassign_head_res = client.put(
        f"/projects/{proj_pid}",
        json={"head_employee_public_id": emp_pid},
        headers=admin_headers,
    )
    assert reassign_head_res.status_code == 200
    assert reassign_head_res.json()["project_head_public_id"] == emp_pid
    assert reassign_head_res.json()["head_employee_name"] is not None

    # 3c. Remove Project Head via null
    remove_null_res = client.put(
        f"/projects/{proj_pid}",
        json={"project_head_public_id": None},
        headers=admin_headers,
    )
    assert remove_null_res.status_code == 200
    assert remove_null_res.json()["project_head_public_id"] is None
    assert remove_null_res.json()["head_employee_name"] is None
