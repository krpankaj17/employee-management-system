...
def test_leave_lifecycle_flow(
    ...
    # 1b. Regular employee can list leave types to select for requests
    list_types_res = client.get("/leaves/types", headers=employee_headers)
    assert list_types_res.status_code == 200
    assert any(lt["public_id"] == leave_type_pid for lt in list_types_res.json())
    ...
    # 3. Query Employee Leave Balances (by UUID and self-service /me)
    bal_res = client.get(
    ...
    me_bal_res = client.get(
        "/leaves/balances/me?year=2026",
        headers=employee_headers,
    )
    assert me_bal_res.status_code == 200
    assert len(me_bal_res.json()) >= 1
